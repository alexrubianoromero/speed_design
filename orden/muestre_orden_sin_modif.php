...
pagada,
saldo
 from $tabla14  where id_empresa = '".$_SESSION['id_empresa']."' and tipo_orden < '2' 
and anulado < 1
   order by id desc";
...
echo '<table border= "1">';
	...
	echo '<td><h3>No Orden<h3></td><td><h3>Imagenes</h3></td><td><h3>Estado</h3></td><td><h3>Linea</h3></td><td><h3>Fecha</h3></td><td><h3>Placa</h3></td><td><h3>Tecnico</h3></td>	
	';
		...
		while($ordenes = mysql_fetch_array($consulta_ordenes))
			{
				...
				echo  '<td><h3>';
				//normalmente el saldo de la orden seria $saldo_orden;
				//pero spped_design pidio que  simplemente se ponga el saldo en cero cuando se coloque 
				//en estado entregada , entonces simplemente se trae el valor de saldo ya no se toma
				//la suma de los items 
				//echo $saldo_orden;
				echo $ordenes['9']; //por la modificacion que pidio spped desing
				echo '</h3></td>';
